...
--
-- Client: localhost
-- Généré le: Dim 09 Février 2014 à 04:25
...
CREATE TABLE IF NOT EXISTS `sk_modules` (
  ...
  `independant_tpl` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
...
INSERT INTO `sk_modules` (`id`, `name`, `url`, `active`, `independant_tpl`) VALUES
(1, 'Sk_Contact', '/contact/', 1, 0),
(2, 'SK_Administrator', '/admin/', 1, 1);
...
(2, 'Page 404', '<p>Cette page n&rsquo;est malheureusement pas disponible actuellement.<br />Le lien que vous avez suivi peut &ecirc;tre incorrect ou la page peut avoir &eacute;t&eacute; supprim&eacute;e.</p>', '/404/'),
(6, 'test', '<p>test</p>', '/test/');

-- --------------------------------------------------------

--
-- Structure de la table `sk_ressources`
--

CREATE TABLE IF NOT EXISTS `sk_ressources` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `type` enum('image','video','url') COLLATE utf8_bin NOT NULL,
...
--
-- Contenu de la table `sk_ressources`
--

INSERT INTO `sk_ressources` (`id`, `type`, `title`, `description`, `url`, `height`, `width`) VALUES
(1, 'image', 'Logo', 'logo de société', 'logo.png', '153', '300');
